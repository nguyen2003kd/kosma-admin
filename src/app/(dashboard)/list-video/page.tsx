...
const Page: React.FC = () => {
  ...
  const [editingVideo, setEditingVideo] = useState<LibraryFile | null>(null)
  const [deletingVideo, setDeletingVideo] = useState<LibraryFile | null>(null)
  ...
  const {
    ...
    queryKey: ['files', 'library-video', pageSize, searchTerm],
    queryFn: async ({ pageParam = 1, signal }) => {
      const res = await getApiV10File(
        {
          page: pageParam,
          pageSize,
          filters: buildVideoFilter(searchTerm),
  ...
  const videos: LibraryFile[] = React.useMemo(() => {
  ...
  return (
    <div>
      <Header title="Kho video" />
      <main className="container mx-auto p-4 md:p-6">
        <div className="space-y-8">
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
            <div>
              <h2 className="text-3xl font-bold tracking-tight">Kho lưu trữ video</h2>
              <p className="text-muted-foreground mt-1">Quản lý video của bạn ({totalCount} video)</p>
            </div>
          ...
          {isLoading ? (
            ...
            <div className="flex items-center justify-center h-64 bg-gray-50 rounded-lg border-2 border-dashed border-gray-300">
              <div className="text-center">
                ...
                <p className="text-gray-400 text-sm">Hãy bắt đầu bằng cách thêm video đầu tiên</p>
              </div>
            </div>
          ) : (
            <>
              <VideoGrid
                videos={videos}
                onEdit={(video) => {
                  if (ability.can('update', 'gallery_video')) setEditingVideo(video)
                }}
                onDelete={(video) => {
                  if (ability.can('delete', 'gallery_video')) setDeletingVideo(video)
                }}
              />

              <div className="flex items-center justify-between mt-4">
                <div className="text-sm text-gray-600">Hiển thị {videos.length} / {totalCount} video</div>
                <div className="text-sm text-gray-600">
                  {isFetchingNextPage ? 'Đang tải thêm...' : hasNextPage ? 'Kéo để tải thêm' : 'Đã tải hết video'}
                </div>
              </div>
